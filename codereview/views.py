...
def _paginate_issues(page_url,
  ...
  if more:
    del issues[limit:]
    next = _url(page_url, offset=offset + limit, **nav_parameters)
  else:
    next = ''
  if offset > 0:
    prev = _url(page_url, offset=max(0, offset - limit), **nav_parameters)
  else:
    prev = ''
  newest = ''
  if offset > limit:
    newest = _url(page_url, **nav_parameters)
 
  _optimize_draft_counts(issues)
  _load_users_for_issues(issues)
  params = {'issues': issues, 'limit': limit,
